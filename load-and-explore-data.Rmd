...
setnames(x6,"Sample.ID.","Sample.ID")
setnames(x6,"Antigen","Ag")
setnames(x6,"Collection.date","Sampling.date")
x6[,nm:="201216"]
x6[,OD:=as.numeric(OD)]
head(x6)

xcomb <- rbind(x[redo==FALSE][,redo:=NULL], x5, x6)
data <- c(data[-1], list(xcomb))  %>% rbindlist(., fill=TRUE)
...
mm3 <- merge(m[group %in% c("200702") & Ag=="SPIKE"],
            m[group %in% c("200702") & Ag=="RBD"],
            by=c("group","Sample.ID","type","stype"),
            suffixes=c(".spike",".rbd"))[,rowid.spike:=NULL][,rowid.rbd:=NULL]
...
## save
save(m, file=file_rdata_v6, version=2)
